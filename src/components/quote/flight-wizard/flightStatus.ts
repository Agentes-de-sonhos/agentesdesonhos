 ...
 */
export function analyzeFlight(data: any, _totalAmount?: number | null): FlightAnalysis {
  const missing: string[] = [];
  if (!data) {
    return {
      status: 'incomplete',
      missing: ['companhia aérea', 'cidade de origem', 'cidade de destino', 'data de ida'],
    };
  }
  ...
  // Valor é opcional: zero é um valor válido e não afeta a completude.
