...
export function QuickLabelPicker({ opportunityId, open, onOpenChange }: QuickLabelPickerProps) {
  const { labels, createLabel } = useOpportunityLabels();
  ...
  const appliedIds = useMemo(
    () => new Set((byOpportunity[opportunityId] || []).map((l) => l.id)),
    [byOpportunity, opportunityId]
  );
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (created?.id) {
        await assignLabel({ opportunityId, labelId: created.id });
      }
      setSearch("");
    } finally {
      setIsCreating(false);
          ...
          {filtered.map((label) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "w-full flex items-center gap-2 px-2 py-2 rounded-md text-left transition-colors hover:bg-muted"
                )}
              >
                <span
                  ...
                  style={{
                    ...
                    color: textColorFor(label.color),
                  }}
                >
                  {label.name}
                </span>
