...
function DayRow({ day, canDelete, onDelete }: { day: ItineraryDay; canDelete: boolean; onDelete: () => void }) {
  ...
  return <div ref={setNodeRef} style={{ transform: CSS.Translate.toString(transform), transition }} className={`flex items-center gap-3 rounded-lg border bg-card p-3 ${isDragging ? "opacity-60 shadow-lg" : ""}`}>
    ...
    <Button type="button" variant="ghost" size="icon" className="h-9 w-9 shrink-0 text-muted-foreground hover:text-destructive" onClick={onDelete} disabled={!canDelete} aria-label={`Excluir Dia ${day.dayNumber}`}><Trash2 className="h-4 w-4" /></Button>
  </div>;
}
...
export function ItineraryDaysOrganizer({ days, startDate, endDate, onReorder, onAdd, onDelete }: Props) {
  ...
  return <div className="space-y-4" data-testid="itinerary-days-organizer">
    ...
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={dragEnd}><SortableContext items={order} strategy={verticalListSortingStrategy}><div className="space-y-2">{order.map(id => { const day = byId.get(id); return day ? <DayRow key={id} day={day} canDelete={days.length > 1} onDelete={() => setDeleteDay(day)} /> : null; })}</div></SortableContext></DndContext>
