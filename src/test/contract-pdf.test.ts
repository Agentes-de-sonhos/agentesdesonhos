  ...
  it('gera payload e PDF sem dados confidenciais', async () => {
    const payload = buildContractPayload({
      ...
      overrides: {
        ...
        installments_count: 3, installment_value: 7140.55, first_due_date: '2026-10-05',
...
describe('modelo Travel.IN Viagens', () => {
  it('gera PDF com as cláusulas 1 a 12 integrais', async () => {
    ...
    const payload = buildContractPayload({
      ...
      overrides: { client_document: '123.456.789-09', down_payment: 10000, payment_method: 'Cartão de crédito parcelado', installments_count: 2, installment_value: 10675.45, first_due_date: '2026-10-05', insurance_contracted: false, insurance_refusal_ack: true },
      contractNumber: buildContractNumber(sale.id, 1),
      revision: 1,
    });
    expect(validateContractPayload(payload).filter((i) => i.severity === 'error')).toEqual([]);
