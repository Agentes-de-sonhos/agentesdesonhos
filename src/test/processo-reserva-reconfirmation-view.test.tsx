...
vi.mock("@/hooks/useTravelFiles", () => ({
  ...
  useTravelFileMutations: () => ({
    ...
    saveService: { mutateAsync: mocks.saveService },
    saveManualData: { mutateAsync: vi.fn() },
    saveManualService: { mutateAsync: vi.fn() },
  }),
  useTravelFileNotes: () => ({
    notes: [],
    addNote: { mutateAsync: mocks.addNote, isPending: false },
...
describe("ProcessoReserva — reconfirmação V2 sem financeiro pré-venda", () => {
  beforeEach(() => {
  ...
  it("não renderiza custo, comissão, margem ou regra financeira no estado do processo 0000005", async () => {
    ...
    expect(screen.getByText("Serviços para reconfirmar")).toBeInTheDocument();
    expect(screen.getAllByText("Solicitado").length).toBeGreaterThan(0);
    expect(screen.getAllByText("Reconfirmado").length).toBeGreaterThan(0);
    expect(screen.getAllByText("Vendido").length).toBeGreaterThan(0);
    expect(screen.getByText("Variação vs. solicitado")).toBeInTheDocument();
    expect(screen.getByText("Fornecedor: Sakura Consolidadora")).toBeInTheDocument();
    expect(screen.getByText("Fornecedor: HOTELDO")).toBeInTheDocument();

    expect(screen.queryByText(/\bCUSTO\b/i)).not.toBeInTheDocument();
    expect(screen.queryByText(/\bCOMISSÃO\b/i)).not.toBeInTheDocument();
    expect(screen.queryByText(/\bMARGEM\b/i)).not.toBeInTheDocument();
    expect(screen.queryByText(/Regra financeira/i)).not.toBeInTheDocument();
    expect(screen.queryByText(/Adicionar informações financeiras/i)).not.toBeInTheDocument();
    expect(screen.queryByText(/Econômica/i)).not.toBeInTheDocument();
    expect(screen.getByRole("button", { name: /Confirmar venda e iniciar operação/i })).not.toBeDisabled();
  });
});