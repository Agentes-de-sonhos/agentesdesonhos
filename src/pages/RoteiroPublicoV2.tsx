  ...
  useEffect(() => {
    ...
    const resolve = async () => {
      try {
        ...
        if (!token) throw new Error(t("notFoundTitle"));
        setShareToken(token);
      } catch (err: any) {
        setError(err.message || t("notFoundTitle"));
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background via-muted/20 to-primary/5 p-4">
        <div className="text-center space-y-2">
          <p className="text-lg font-medium text-destructive">{error || t("notFoundTitle")}</p>
          <p className="text-sm text-muted-foreground">{t("checkLinkMessage")}</p>
        </div>
      </div>
    );
  }
