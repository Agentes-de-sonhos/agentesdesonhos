...
BEGIN
  ...
  -- Atividades: eventos da agenda + follow-ups (hoje / próximos dias)
  CREATE TEMP TABLE IF NOT EXISTS _wl_dash_activities (
    kind text, id text, link_id text, title text, subtitle text,
    type_label text, activity_date date, activity_time time, all_day boolean, overdue boolean
  ) ON COMMIT DROP;
  DELETE FROM _wl_dash_activities;

  IF _can_agenda THEN
    INSERT INTO _wl_dash_activities
    SELECT 'event', e.id::text, e.id::text,
           COALESCE(NULLIF(e.title, ''), 'Compromisso'),
           NULLIF(btrim(COALESCE(c.name, '') || CASE WHEN c.name IS NOT NULL AND NULLIF(e.description,'') IS NOT NULL THEN ' · ' ELSE '' END || COALESCE(left(e.description, 90), '')), ''),
           COALESCE(NULLIF(e.event_type, ''), 'Agenda'),
           e.event_date, e.event_time, COALESCE(e.all_day, false), false
      FROM public.agency_events e
      LEFT JOIN public.clients c ON c.id = e.client_id
     WHERE e.user_id = ANY(_owners)
       AND e.deleted_at IS NULL
       AND e.event_date >= _today - 30
       AND e.event_date <= _today + 120;
  END IF;
  ...
  IF _can_opp THEN
    ...
    SELECT DISTINCT ON (fu.opportunity_id, fu.follow_up_date)
           ...
           NULLIF(btrim(COALESCE(o.destination, '') || CASE WHEN o.destination IS NOT NULL AND NULLIF(fu.note,'') IS NOT NULL THEN ' · ' ELSE '' END || COALESCE(left(fu.note, 90), '')), ''),
           'Follow-up',
           fu.follow_up_date, (fu.follow_up_at AT TIME ZONE _tz)::time, false,
     ...
     WHERE o.user_id = ANY(_owners)
       ...
       AND fu.follow_up_date >= _today - 60
       AND fu.follow_up_date <= _today + 120
     ORDER BY fu.opportunity_id, fu.follow_up_date, fu.created_at;
  END IF;

  -- Hoje (inclui follow-ups vencidos, que continuam sendo tarefa do dia)
  SELECT COALESCE(jsonb_agg(to_jsonb(t) ORDER BY t.activity_date, t.activity_time NULLS LAST), '[]'::jsonb)
    INTO _today_items
  FROM (
    SELECT * FROM _wl_dash_activities
     WHERE activity_date <= _today
    ...
    FROM (
      SELECT tr.id::text AS id,
       ...
       WHERE tr.user_id = ANY(_owners)
         AND tr.start_date >= _today
         AND tr.start_date <= _today + 60
