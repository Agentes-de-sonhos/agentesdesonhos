...
AS $function$
DECLARE
  trip_record RECORD; services_data json; agent_profile json;
BEGIN
  ...
  RETURN json_build_object('trip', json_build_object(
    'id', trip_record.id, 'client_name', trip_record.client_name, 'trip_title', trip_record.trip_title,
...
CREATE OR REPLACE FUNCTION public.verify_trip_access_by_slug(p_slug text, p_password text)
 RETURNS json LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $function$
DECLARE trip_record RECORD; services_data json; agent_profile json;
BEGIN
...
CREATE OR REPLACE FUNCTION public.get_trip_by_public_code(p_agency_slug text, p_code text)
 RETURNS json LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $function$
DECLARE trip_record RECORD; agent_record RECORD; services_data json; agent_profile json; agency_slug_check text;
BEGIN
