...
CREATE OR REPLACE FUNCTION private.agency_master_record_visible(_record_user_id uuid)
RETURNS boolean
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION private.agency_master_record_visible(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION private.agency_master_record_visible(uuid) TO authenticated, service_role;
...
CREATE POLICY master_agency_quotes_all ON public.quotes
  ...
  USING (private.agency_master_record_visible(user_id))
  WITH CHECK (private.agency_master_record_visible(user_id));
...
CREATE POLICY master_agency_quote_services_all ON public.quote_services
  ...
  USING (EXISTS (
    ...
    WHERE q.id = quote_services.quote_id
      AND private.agency_master_record_visible(q.user_id)
  ))
  WITH CHECK (EXISTS (
    SELECT 1 FROM public.quotes q
    WHERE q.id = quote_services.quote_id
...
-- Carteiras digitais: conteúdo interno.
...
CREATE POLICY master_agency_trip_services_all ON public.trip_services
  ...
  USING (EXISTS (
    ...
    WHERE t.id = trip_services.trip_id
      AND private.agency_master_record_visible(t.user_id)
  ))
  WITH CHECK (EXISTS (
    SELECT 1 FROM public.trips t
    WHERE t.id = trip_services.trip_id
...
CREATE POLICY master_agency_itinerary_days_all ON public.itinerary_days
  ...
  USING (EXISTS (
    ...
    WHERE i.id = itinerary_days.itinerary_id
      AND private.agency_master_record_visible(i.user_id)
  ))
  WITH CHECK (EXISTS (
    SELECT 1 FROM public.itineraries i
    WHERE i.id = itinerary_days.itinerary_id
...
-- Remove a versão pública da função auxiliar, agora substituída pela privada.
DROP FUNCTION IF EXISTS public.agency_master_record_visible(uuid);