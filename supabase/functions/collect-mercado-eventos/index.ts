...
serve(async (req) => {
  ...
  try {
    const result = await runCollectPortal("Mercado & Eventos", auth.triggerSource);
    return new Response(JSON.stringify(result), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (e) {
    console.error("collect-mercado-eventos error:", e);
    return new Response(JSON.stringify({ error: "Falha ao coletar Mercado & Eventos." }), {
      status: 500,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
