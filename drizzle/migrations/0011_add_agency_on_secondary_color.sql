ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS agency_on_secondary_color text;

COMMENT ON COLUMN public.profiles.agency_on_secondary_color IS
  'HEX opcional do texto/icones exibidos sobre a cor secundaria da agencia. NULL = contraste automatico (aparencia atual).';
...
RETURNS TABLE(
  ...
  agency_on_secondary_color text
)
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $function$
  SELECT jsonb_build_object(
    ...
    'on_secondary_color', p.agency_on_secondary_color,
    'phone', p.phone,
    'city', p.city,
    'state', p.state,
    'bio', p.bio,
    ...
    'cnpj', p.cnpj,
    'whatsapp_group_url', p.agency_whatsapp_group_url
  )
