-- Fase 1A (2ª rodada corretiva).
-- 1) Lock determinístico da oportunidade: dois processos diferentes da mesma
--    oportunidade não podem mais confirmar venda em paralelo.
-- 2) Recebimento do cliente no V2 calculado só pela linhagem do processo.
-- Nada aqui ativa o fluxo unificado (entitlement continua desligado).

-- ============================================================
-- Recebimento do cliente: linhagem exclusiva no fluxo V2
-- ============================================================
...
BEGIN
  ...
  -- Operação do fluxo unificado: a base tem vendas legadas duplicadas na mesma
  -- oportunidade, então o V2 nunca soma por opportunity_id.
  _v2 := _op.travel_file_id IS NOT NULL OR coalesce(_op.flow_origin, '') = 'travel_file_v2';

  -- Faturas legitimamente ligadas à operação (direto, pelo orçamento ou pela
  -- oportunidade) continuam valendo nos dois fluxos.
  SELECT COALESCE(sum(i.total_amount), 0), COALESCE(sum(i.total_amount - i.balance), 0)
    INTO _invoiced, _paid
  ...
  -- Vendas desta operação. No V2, somente pela linhagem (operação e processo).
  SELECT array_agg(s.id) INTO _sale_ids
  FROM public.sales s
  WHERE s.user_id = _op.user_id
    AND (
      s.source_operation_id = _op.id
      OR (_op.travel_file_id IS NOT NULL AND s.travel_file_id = _op.travel_file_id)
      OR (NOT _v2 AND s.opportunity_id IS NOT NULL AND s.opportunity_id = _op.opportunity_id)
    );

  IF _total <= 0 THEN
    SELECT COALESCE(sum(s.sale_amount), 0) INTO _total
    FROM public.sales s WHERE s.id = ANY(COALESCE(_sale_ids, '{}'::uuid[]));
  END IF;

  -- Recebimentos diretos. Pagamentos originados de fatura já entraram acima
  -- pelo saldo da fatura e não podem ser contados duas vezes.
  SELECT _paid + COALESCE(sum(cp.amount), 0) INTO _paid
  FROM public.customer_payments cp
  WHERE cp.user_id = _op.user_id
    AND NOT (_invoiced > 0 AND COALESCE(cp.source, 'manual') = 'invoice')
    AND (
      (COALESCE(cp.source, '') = 'operation' AND cp.source_id = _op.id)
      OR cp.sale_id = ANY(COALESCE(_sale_ids, '{}'::uuid[]))
    );
...
COMMENT ON FUNCTION public.compute_operation_customer_payment_status(uuid) IS
  'Situação do recebimento do cliente. No fluxo unificado (travel_file_id/flow_origin) considera somente a linhagem da operação e do processo, nunca vendas agrupadas por oportunidade.';

-- ============================================================
-- Confirmação da venda: lock file -> lock oportunidade -> checagens
-- ============================================================
...
BEGIN
  ...
  -- LOCK 2: a oportunidade. Dois processos diferentes da MESMA oportunidade
  -- serializam aqui, então a segunda confirmação enxerga o vínculo criado pela
  -- primeira e falha inteira em WORKFLOW_LINK_CONFLICT.
...
COMMENT ON FUNCTION public.confirm_travel_file_sale(uuid, text, jsonb, timestamptz) IS
  'Confirmação transacional e idempotente da venda de um processo de reserva (fluxo unificado V2). Locks determinísticos: travel_files depois opportunities, com todas as checagens de vínculo revalidadas sob os dois locks.';
